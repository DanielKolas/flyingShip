...
createScore(){
    ...
    this.add.text(16, 60, `Best score: ${bestScore || 0}`, {fontSize: "32px", color:"#000"});
}
...
checkGameStatus(){
    if(this.bird.body.position.y >= this.config.height - this.bird.body.height || this.bird.body.position.y <= 0){
        this.gameOver();
    }
}
...
placePipe(pipeUpper, pipeLower) {
    ...
    const pipeVerticalPosition = Phaser.Math.Between(20, this.config.height-pipeVerticalDistance -20)
    const pipeHorizontalDistance = Phaser.Math.Between(...this.pipeHorizontalRange);
